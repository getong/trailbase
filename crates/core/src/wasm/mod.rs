...
pub(crate) use trailbase_wasm_runtime_host::functions::SqliteFunctions;
...
pub(crate) async fn build_sync_wasm_runtimes_for_components(
  ...
  use_winch: bool,
) -> Result<Vec<(Arc<Mutex<SqliteStore>>, SqliteFunctions)>, AnyError> {
  ...
  let mut sync_runtimes: Vec<(Arc<Mutex<SqliteStore>>, SqliteFunctions)> = vec![];
  ...
  for path in components {
    ...
    if !functions.scalar_functions.is_empty() {
      sync_runtimes.push((Arc::new(Mutex::new(store)), functions));
    }
  }
